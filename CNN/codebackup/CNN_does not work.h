...
public:
	...
	Weights(int mapX, int mapY, string type1){
		...
		if(type1 == "s"){
			...
			flag = rand()%100;
			if(flag%2==0){
				weights[0][0] = ((double)rand()/RAND_MAX)*(.5);
			}else{
				weights[0][0] = (-(double)rand()/RAND_MAX)*(.5);
			} 
			flag = rand()%100;
			if(flag%2==0){
				bias= ((double)rand()/RAND_MAX)*(.5);
			}else{
				bias = (-(double)rand()/RAND_MAX)*(.5);
			} 
		...
		if(type1 == "c"){
			...
			for(int i = 0; i<mapX; i++){
				for(int j = 0; j<mapY; j++){
					flag = rand()%100;
					if(flag%2==0){
						weights[i][j] = ((double)rand()/RAND_MAX)*(.5);
					}else{
						weights[i][j] = (-(double)rand()/RAND_MAX)*(.5);
					}
				}
...
public:
			...
			if(layers[i].getType() == "c" || layers[i].getType() == "s"){
				...
				for(int j = 0; j<layers[i].numOfMaps; j++){
					if(layers[i].getType() != "s"){
						...
						layers[i].data[j] = *(new Weights(2, 2, layers[i].getType()));
					}
				}
			}
		...
		for (int i = 1; i<numLayers; i++){
			if(layers[i].getType() == "c" || layers[i].getType() == "s"){
				...
				for (int j = 0; j<layers[i].numNeurons; j++){
					...
					if(layers[i].getType() == "s"){
						//multiply by BETA
						layers[i].neurons[j].output *= layers[i].data[layers[i].neurons[j].featureMapID].weights[0][0];
						//add the bias
						layers[i].neurons[j].output +=layers[i].data[layers[i].neurons[j].featureMapID].bias;
						
					}else if(layers[i].getType() == "c"){
			...
			for (int n = numHidden; n>0; n--){
				if(layers[n].getType() == "r"){
					...
					if(layers[n+1].getType() == "r"){
						...
						for(int i = 0; i<layers[n].numNeurons; i++){
							layers[n].neurons[i].delta = layers[n].neurons[i].errorFactor;
					...
					if(layers[n+1].getType() == "c"){
						...
						for(int i = 0; i<layers[n].numNeurons; i++){
							for(int j = 0; j<layers[n].neurons[i].outputs.size(); j++){
								...
								xpos = weights%size;
								ypos = weights/size;
								layers[n].neurons[i].errorFactor = layers[n+1].neurons[nextLayerID].delta * layers[n+1].data[layers[n+1].neurons[nextLayerID].featureMapID].weights[xpos][ypos];
								weights++;
								weights %= size*size;
							}
						}
					...
					if(layers[n+1].getType() == "s"&&layers[n+2].getType() == "r"){
						...
						for(int i = 0; i<layers[n].numNeurons; i++){
							...
							for(unsigned int j = 0; j<layers[n].neurons[i].outputs.size(); j++){
								...
								layers[n].neurons[i].errorFactor += layers[n+1].neurons[nextLayerID].delta*((double)1/layers[n+1].neurons[nextLayerID].inputs.size());
								//cout << 1/layers[n+1].neurons[nextLayerID].inputs.size() << endl;
						...
						for(int i = 0; i<layers[n].numNeurons; i++){
							...
							for(unsigned int j = 0; j<layers[n].neurons[currentLayerID].outputs.size(); j++){
								...
								layers[n].neurons[currentLayerID].errorFactor += layers[n+1].neurons[nextLayerID].delta*((double)1/layers[n+1].neurons[nextLayerID].inputs.size());
								if(n==3) {
									cout << "hi\n";
								}
	...
	void train(int num_epochs){
		...
		for(int i = 0; i<num_epochs; i++){
			...
			layers[numLayers-1].neurons[input_data[id].subject_num-1].expected = 1.0f;
			int size = 0;
			...
			//propagate();
			learningRate += -learningRate/(numIterations);
		}
