...
int main(int argc, char* argv[])
{
	...
	for(int i = 0; i<22; i++){
		for(int j = 0; j<27; j++){
			pixel.Red = c.map(c.layers[1].neurons[count].output, -1, 1, 0, 1) * 255;
			pixel.Green = c.map(c.layers[1].neurons[count].output, -1, 1, 0, 1) * 255;
			pixel.Blue = c.map(c.layers[1].neurons[count].output, -1, 1, 0, 1) * 255;
			pixel.Alpha = 0;
			count ++;
...
#pragma endregion
	...
	for(int z = 0; z<c.layers[1].numOfMaps; z++){
		...
		filename = "intermediate_outputs\\layer_2_image"+c.int_to_str(z+20)+".bmp";
		bitmap.WriteToFile(filename.c_str());
	}
	//ofstream fout;
